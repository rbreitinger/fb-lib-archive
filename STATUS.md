# fb-lib-archive — Library Status

> **FreeBASIC target:** 1.10.1 — all bundles live under `libraries/freebasic-1-10-1/`
>
> **Legend:** ✅ Done | 🔧 In progress | ⬜ Open | ❌ Not available | ❓ Unknown
>
> **Ver** = version the FB 1.10.1 `.bi` header was translated from

---

## External Libraries

### 🔊 Audio

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| OpenAL | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| BASS | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| BASSmod | ❓ | ⬜ | ⬜ | ❌ | ❌ | ⬜ |
| FMOD | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| ModPlug | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| mpg123 | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libogg | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| PortAudio | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libsndfile | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| SoLoud | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libvorbis | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

### 🎨 Graphics / 2D

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| Allegro 4 | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| Allegro 5 | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| Cairo | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| Canvas Draw (CD) | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| FreeImage | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libgd | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| DevIL | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| IM (image lib) | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libart | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libpng | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| giflib | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libjpeg | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| Raylib | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| SDL 1.x | ❓ | ⬜ | ❌ | ⬜ | ⬜ | ⬜ |
| SDL 2.x | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| TinyPTC | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| GDK / GDK-PixBuf | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

### 🟦 OpenGL / Window context

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| OpenGL (system) | — | ✅ | ✅ | ⬜ | ⬜ | ⬜ |
| freeglut | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| GLFW 2 | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| GLFW 3 | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| GtkGLExt | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| Mesa (GL headers) | — | — | — | ⬜ | ⬜ | ⬜ |

### 🖼️ GUI Toolkits

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| CGUI | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| DISLIN | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| Glade | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| GooCanvas | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| GTK 2/3 | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| IUP | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| JAPI | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| wxWidgets (wx-c) | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

### 🔤 Fonts / Text rendering

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| Flite TTS | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| fontconfig | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| FreeType 2 | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| Pango | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

### ⚙️ Physics

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| Chipmunk | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| Newton | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| ODE | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

### 📦 Compression

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| bzip2 | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| LZMA / liblzma | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| LZO | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| QuickLZ | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libzip | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| zlib | 1.2.8 | ✅ | ❌ | ⬜ | ⬜ | ✅ |

### 🗄️ Databases

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| GDBM | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| MariaDB | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| MySQL | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| PostgreSQL | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| SQLite 2 | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| SQLite 3 | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

### 🌐 Networking / Web / IPC

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| CGI-util | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libcurl | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| FastCGI | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| ZeroMQ | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

### 📜 Scripting engines

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| Lua | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| SpiderMonkey (JS) | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

### 📄 XML / JSON / Data formats

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| Expat | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| JSON-C | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libxml2 | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libexslt | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libxslt | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| MiniXML | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| XMP toolkit | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

### 🔢 Math / Science

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| BigInt | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| GMP | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| GSL | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

### 🔍 Regular expressions

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| PCRE (v1) | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| PCRE2 | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| TRE | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

### 🎬 Media / Multimedia

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| MediaInfo | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libVLC | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

### 🔐 Crypto / Security

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| cryptlib | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

### 🌍 Internationalization / Text encoding

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| aspell | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| gettext-po | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libiconv | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libintl | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| pspell | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| utf_conv | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

### 🛠️ Developer tools / Misc

| Library | Ver | Win32 | Win64 | Linux32 | Linux64 | Tested |
|---------|-----|-------|-------|---------|---------|--------|
| BFD (binutils) | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| clang-c | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| CUnit | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| DispHelper | ❓ | ⬜ | — | — | — | ⬜ |
| FFI (libffi) | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| JIT (libjit) | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| JNI (Java) | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| LLVM-C | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| PDFlib | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| UUID | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| GRX | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| GDSL | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |
| libcaca | ❓ | ⬜ | ⬜ | ⬜ | ⬜ | ⬜ |

---

## Platform / OS-provided headers

No binary bundles needed — provided by the OS or built into FB.

| Header group | Notes |
|--------------|-------|
| FB Runtime | `fbgfx.bi`, `fbio.bi`, `fblimits.bi`, `fbprng.bi`, `fbthread.bi`, `fbc-int/` — built into FB |
| FB Compat helpers | `vbcompat.bi`, `datetime.bi`, `string.bi`, `dir.bi`, `file.bi` — built into FB |
| C Runtime wrappers | `crt/` — FB's abstraction over the C stdlib |
| Windows API | `win/`, `windows.bi` — provided by Windows / MinGW |
| X Window System | `X11/`, `xcb/` — provided by Linux X server packages |
| DOS / DJGPP | `dos/` — DOS platform specific |

---

## Next Steps

1. Fill in target versions by inspecting each `.bi` header
2. All bundles go under `libraries/freebasic-1-10-1/`
3. Future FB releases get their own sibling folder
4. Linux builds welcome — see `CONTRIBUTING.md`
